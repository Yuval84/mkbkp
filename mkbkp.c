 ...
 * 24/01/17
 * mkbkp.c -archive and extract files and folder
 * Usage: mkbkp <-c|-x> <backup_file> [file_to_backup|directory_to_backup]
 *
 */
 ...
 * base - base of the path to archive)
 * file - filename
 * archive_file
 */
 ...
 */
int copy_content(FILE* dest, FILE* src, size_t size_src);

/* Writes the given file node's file to the given archive */
void write_to_archive(node_t *header, FILE* archive_file);
...
int main(int argc, char** argv){
        ...
        unsigned int ValidSyntax = 0;
        unsigned int Compressing = 0;

        if (argc > 2){
                /* archive */
                if ((0 == strcmp(COMPRESS, argv[1])) && (argc == 4)){
                        /* Syntax is ok */
                        ValidSyntax = 1;
                        Compressing = 1;
                }
                /* Extract */
                else if ((0 == strcmp(EXTRACT, argv[1])) && (argc == 3))
                {
                        /* Syntax is ok */
                        ValidSyntax = 1;
                        Compressing = 0;
...
void archive(const char* base, const char* file, FILE* archive_file)
{
        ...
        if (file == NULL || archive_file == NULL)
                return;
	...
	if (base){
        ...
        if (0 == lstat(header.path, &status)){
                ...
                if (S_ISLNK(status.st_mode)){
                        ...
                        int linked_path_length = readlink(header.path,content.linked_path,PATH_MAX_LENGTH);

                        /* Make sure the link is valid for us to archive */
                        if (linked_path_length >= PATH_MAX_LENGTH || linked_path_length < 0){
                        ...
                        if (fwrite(&content,sizeof(content), 1, archive_file) != 1) {
                ...
                /* Folder / File */
                        ...
                        if (S_ISDIR(status.st_mode)) {
                                ...
                                /* Make sure we stopped because we went thru
                                 all the files */
                                if (errno) {
                                        fprintf(stderr, "Error while processing directory %s: ", header.path);
                                        perror("");
                                }

                                /* Close down the dir stream */
                                if (closedir(dir)) {
                                        fprintf(stderr, "Cannot close directory %s: ", header.path);
                                        perror("");
                                }
                        }
                        /* File */
                        else {
                        	write_to_archive(&header, archive_file);
                        }
                }
                else {
                        printf("file type not supported for %s: %d\n", header.path,  status.st_mode);
                }
        }
        else{
...
int copy_content(FILE* dest, FILE* src, size_t size_src)
{
        ...
        /* Copy byte by byte */
        while ((byte_read = fgetc(src)) != EOF && (size_src == 0 || bytes_copied < size_src)) {

                if (fputc(byte_read, dest) == EOF) return -1;

                bytes_copied++;

                /* All bytes read */
                if (bytes_copied == size_src) return 0;
        }
...
void write_to_archive(node_t *header, FILE* archive_file)
{
        ...
        if (0 != copy_content(archive_file, curr_file, 0)) {
...
void extract(FILE* archive_file){
        ...
        if (archive_file == NULL) return;
        /* Read the archive entry-by-entry, extracting each one to its correct place.
         */
        while (1 == fread(&header, sizeof(header), 1, archive_file)) {
                ...
                if (S_ISLNK(header.mode)) {
                        symbolic_link_content_t content;
                        /* Extract the link */
                        if (1 != fread(&content,sizeof(content),1, archive_file) || symlink(content.linked_path, header.path) != 0) {
                ...
                /* Handle file */
                        ...
                        if (0 != copy_content(extracted_file,archive_file, header.concrete.type.file.size))
